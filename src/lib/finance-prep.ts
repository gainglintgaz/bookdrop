...
/** Build HTML + filename for the bookkeeper package (no side effects). */
export function buildBookkeeperPackageHtml(options: PackageOptions): { html: string; filename: string } {
  const {
    businessName,
    contactName,
    year,
  ...
  const packageStatusBadge = completeness.readyForBookkeeper
    ? '<span class="badge badge-pass">Package ready for review</span>'
    : '<span class="badge badge-fail">Incomplete — missing required documents</span>'
...
<p style="margin-bottom: 16px;">${packageStatusBadge}</p>
...
<div class="footer">
  ...
  <p>This report is for informational purposes. Verify all data before filing. Not audited statements.</p>
</div>

</body>
</html>`

  const filename = `${businessName.replace(/\s+/g, '_')}_${year}_${String(month).padStart(2, '0')}_package.html`
...
export function generateBookkeeperPackage(options: PackageOptions): void {
  const { html, filename } = buildBookkeeperPackageHtml(options)
