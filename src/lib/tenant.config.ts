...
//
// Change VITE_TENANT_VERTICAL in .env to switch verticals.
...
const configs: Record<string, TenantConfig> = {
  ...
  bookkeeping: {
    ...
    taxDocumentTypes: [
      { label: 'W-2 (Wage & Tax Statement)', doc_type: 'w2', required: true },
      { label: 'Pay Stubs (all)', doc_type: 'payroll', required: true },
      { label: '1099-NEC (Non-Employee Compensation)', doc_type: '1099_nec', required: false },
      { label: '1099-MISC (Miscellaneous Income)', doc_type: '1099_misc', required: false },
      { label: '1099-INT (Interest Income)', doc_type: '1099_int', required: false },
      { label: '1099-DIV (Dividend Income)', doc_type: '1099_div', required: false },
      { label: '1099-K (Payment Card Transactions)', doc_type: '1099_k', required: false },
      { label: 'Last Year 1040 / Tax Return', doc_type: '1040', required: false },
      { label: 'Mortgage Interest Statement (1098)', doc_type: '1098', required: false },
      { label: 'Investment Statements (Brokerage)', doc_type: 'investment', required: false },
      { label: 'Bank Statement (Primary Account)', doc_type: 'bank', required: true },
      { label: 'Credit Card Statement', doc_type: 'credit_card', required: true },
      { label: 'Invoices / Receipts over $75', doc_type: 'receipt', required: false },
    ],
...
export const useIsVertical = (vertical: string): boolean => VERTICAL === vertical
export const usePlanLimit = (plan: 'free' | 'starter' | 'pro'): number | null =>
  tenantConfig.planLimits[plan]
export const useFeature = (feature: keyof TenantConfig['features']): boolean =>
