...
function generateAdvice(
  ...
  if (uncategorized && uncategorized.percentage > 30) {
    advice.push({
      ...
      description: `${uncategorized.count} transactions totaling $${uncategorized.total.toFixed(0)} couldn't be auto-categorized. Review these for proper categorization before tax filing.`,
      impact: 'medium',
    })
  }
  ...
  if (vendors.length > 0) {
    ...
    if (topVendorPct > 30) {
      advice.push({
        ...
        description: `"${vendors[0].name}" accounts for $${vendors[0].total.toFixed(0)} of total expenses. Consider whether this concentration is healthy for the business.`,
        impact: 'medium',
      })
    }
  }
  ...
  if (recurring.length > 5) {
    ...
    advice.push({
      ...
      description: `Monthly recurring expenses total ~$${totalRecurring.toFixed(0)}. Review for unused subscriptions or services that could be consolidated.`,
      impact: 'medium',
    })
  }
  ...
  if (cashFlow.netCashFlow > 0 && cashFlow.totalIncome > 0) {
    ...
    if (margin > 20) {
      advice.push({
        ...
        description: `Net cash flow of $${cashFlow.netCashFlow.toFixed(0)} this month. Consider setting aside ${Math.min(margin * 0.3, 25).toFixed(0)}% for estimated quarterly taxes.`,
        impact: 'low',
      })
    }
  }
...
<style>
  ...
  ${advice.length > 0 ? `<h2>Insights & Recommendations</h2>${adviceRows}` : ''}
